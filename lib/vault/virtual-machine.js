...
async function createVirtualMachine(input) {
  ...
  const clusterCaCert = input.kubeconfig.apply((kubeconfig) =>
    Buffer.from(
      (0, yaml_1.parse)(kubeconfig).clusters[0].cluster[
  ...
  const clusterServer = input.kubeconfig.apply(
    (kubeconfig) => (0, yaml_1.parse)(kubeconfig).clusters[0].cluster['server'],
  );
  ...
  const virtualMachine = new azure_native.compute.VirtualMachine(
    'vault-vm',
    {
      ...
      osProfile: {
        ...
        computerName: 'vault',
        customData: (0, cloud_init_1.createCloudInitCustomData)({
