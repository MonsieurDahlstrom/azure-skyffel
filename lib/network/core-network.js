...
async function setup(input) {
  ...
  const subnetPromises = input.layout.subnets.map(async (snet) => {
    const subnet = new azure.network.Subnet(snet.name, {
      ...
      virtualNetworkName: exports.virtualNetwork.name,
      addressPrefix: snet.cidr,
      delegations: getDelegation(snet.delegationType),
    });
    exports.subnets.set(snet.name, subnet);
  });
...
function reset() {
  exports.virtualNetwork = undefined;
  exports.subnets = new Map();
}
function getDelegation(delegationType) {
  const delegation =
