...
async function createPrivateDnsZone(input) {
    const privateDnsZone = new azure_native.network.PrivateZone(`private-dns-zone-${input.key}`, {
        location: 'Global',
        privateZoneName: input.zone,
        resourceGroupName: input.resourceGroup.name,
    }, { dependsOn: [input.network, input.resourceGroup] });
    const link = new azure_native.network.VirtualNetworkLink(`vnet-link-${input.key}`, {
        location: 'Global',
...
async function createAddressEntry(input) {
    const recordSet = pulumi
        ...
        return new azure_native.network.PrivateRecordSet(`arecord-${input.name}-${dnsZoneName}`, {
            aRecords: [
                {
