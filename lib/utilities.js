...
function cidrSubnet(iprange, newbits, netnum) {
    if (ipaddress.Address4.isValid(iprange)) {
        ...
        const newAddresBI = addressBI.add(twoBI.pow(32 - newsubnetMask).multiply(netnumBI));
        ...
        const twoBI = new jsbn_1.BigInteger('2');
        const netnumBI = new jsbn_1.BigInteger(netnum.toString());
        const newAddresBI = addressBI.add(twoBI.pow(128 - newsubnetMask).multiply(netnumBI));
        const newAddress = ipaddress.Address6.fromBigInteger(newAddresBI).correctForm();
        return newAddress + '/' + newsubnetMask;
    }
    else {
