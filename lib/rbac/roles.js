...
exports.assignRoleOutput = assignRoleOutput;
...
function assignRole(input) {
    const roleGUID = new random.RandomUuid(`${input.principal.type}-${input.principal.id}-assigned-role-${input.rbacRole}-for-${input.key}`, {});
    return new azure_native.authorization.RoleAssignment(`role-${input.rbacRole}-assiged-to-${input.principal.id}-for-${input.scope}`, {
...
function assignRoleOutput(input) {
    return input.scope.apply((scope) => {
        const roleGUID = new random.RandomUuid(`${input.principal.type}-${input.principal.id}-assigned-role-${input.rbacRole}-for-${input.key}`, {});
        return new azure_native.authorization.RoleAssignment(`role-${input.rbacRole}-assiged-to-${input.principal.id}-for-${scope}`, {
            principalId: input.principal.id,
            principalType: input.principal.type,
            roleAssignmentName: roleGUID.result,
            roleDefinitionId: `/subscriptions/${input.subscriptionId}/providers/Microsoft.Authorization/roleDefinitions/${input.rbacRole}`,
