...
async function create(input) {
    const randomKeyVaultName = new Random.RandomString(`kv-${input.name}-randomizer`, {
    ...
    const keyVault = new azure_native.keyvault.Vault(`kv-${input.name}`, {
        vaultName: randomKeyVaultName.result.apply((suffix) => `kv-${input.name}-${suffix}`),
    ...
    const keyVaultPrivateEndpoint = new azure_native.network.PrivateEndpoint(`kv-pe-${input.name}`, {
        location: input.resourceGroup.location,
        resourceGroupName: input.resourceGroup.name,
    ...
    const keyVaultPrivateDnsZoneGroup = new azure_native.network.PrivateDnsZoneGroup(`kv-pdzg-${input.name}`, {
        resourceGroupName: input.resourceGroup.name,
